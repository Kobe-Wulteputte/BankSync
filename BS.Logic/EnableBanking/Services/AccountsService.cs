...
namespace EnableBanking.Services
{
    public class AccountsService : HttpClientService, IAccountsService
    {
        ...
        {
            var requestUri = $"accounts/{request.AccountId}/transactions?";
            if (request.DateFrom != null)
            {
                requestUri += $"date_from={request.DateFrom.Value:yyyy-MM-dd}&";
            }

            if (request.DateTo != null)
            {
                requestUri += $"date_to={request.DateTo.Value:yyyy-MM-dd}&";
            }
        ...
        {
            var requestUri = $"accounts/{request.AccountId}/transactions/{request.TransactionId}";
        ...
        {
            var requestUri = $"accounts/{request.AccountId}/details";
        ...
        {
            var requestUri = $"accounts/{request.AccountId}/balances";
